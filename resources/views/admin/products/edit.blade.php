...
<div class="table-container rounded-lg p-6">
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="form-section">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $product->name ?? $product->title) }}" class="w-full border rounded-lg p-3" required>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="stock" value="{{ old('stock', $product->stock) }}" class="w-full border rounded-lg p-3" required>
                </div>
                <div>
                    <label class="font-semibold mb-2 block">Brand</label>
                    <input type="text" name="brand" value="{{ old('brand', $product->brand) }}" class="w-full border rounded-lg p-3">
                </div>
            </div>
        </div>
        ...
        <div class="form-section">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="material" value="{{ old('material', $product->material) }}" class="w-full border rounded-lg p-3">
                </div>
                <div>
                    <label class="font-semibold mb-2 block">Dimensions</label>
                    <input type="text" name="dimensions" value="{{ old('dimensions', $product->dimensions) }}" class="w-full border rounded-lg p-3">
                </div>
            </div>
            <div class="mt-4">
                <label class="font-semibold mb-2 block">Description</label>
                <textarea name="description" rows="3" class="w-full border rounded-lg p-3">{{ old('description', $product->description) }}</textarea>
            </div>
            <div class="mt-4">
                <label class="font-semibold mb-2 block">Care Instructions</label>
                <textarea name="care_instructions" rows="3" class="w-full border rounded-lg p-3">{{ old('care_instructions', $product->care_instructions) }}</textarea>
            </div>
        </div>
        ...
        <div class="form-section">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="font-semibold mb-2 block">Upload New Image</label>
                    <input type="file" name="image" class="w-full border rounded-lg p-3">
                </div>
                @if($product->image)
                <div>
                    <label class="font-semibold mb-2 block">Current Image</label>
            ...
            <div id="features-container">
                @if(old('features', $product->features))
                    ...
                    <div class="flex gap-2 mb-2 feature-item">
                        <input type="text" name="features[]" value="{{ $feature }}" class="border rounded-lg p-3 w-full" placeholder="Feature description">
        ...
        <div class="flex justify-between border-t pt-6">
            ...
            <button type="submit" class="btn-save px-6 py-3 rounded-lg font-semibold">Update Product</button>
        </div>
    </form>
</div>
